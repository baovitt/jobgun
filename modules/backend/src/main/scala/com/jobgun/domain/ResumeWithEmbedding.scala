...
final case class ResumeWithEmbeddings(
    data: SuperParserResponse
) derives JsonCodec

object ResumeWithEmbeddings:
  import zio.*
  import zio.openai.model.OpenAIFailure
  import com.jobgun.shared.model.EmbeddingModel

  def fromDescription(
      jobDescription: SuperParserResponse
  ): ZIO[EmbeddingModel, OpenAIFailure, ResumeWithEmbeddings] =
    // val educationRequirements = 
    //     EmbeddingModel.embedText(jobDescription.educationRequirements.toString).map { embeddings =>
